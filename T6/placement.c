...
void placeAutonomously(AutonomousPlayer players[], Board *board, int my_id, int num_players)
{
    ...
    for (int i = 0; i < board->rows; i++)
    {
        for (int j = 0; j < board->columns; j++)
        {
            ...
            {
                int score = 1;
                if (i + 1 < board->rows && board->array[i + 1][j] > 0)
                    score *= board->array[i + 1][j];
                if (i - 1 >= 0 && board->array[i - 1][j] > 0)
                    score *= board->array[i - 1][j];
                if (j + 1 < board->columns && board->array[i][j + 1])
                    score *= board->array[i][j + 1];
                if (j - 1 >= 0 && board->array[i][j - 1])
                    score *= board->array[i][j - 1];
                if (score >= best_score)
                {
                    best_score = score;
